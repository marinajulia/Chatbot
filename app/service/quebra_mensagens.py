...
def ajustar_sentencizer(nlp_model):
    """
    ...
    for abbr in abbreviations:
        if ORTH is not None:
            nlp_model.tokenizer.add_special_case(abbr, [{ORTH: abbr}])
    
    # Adicionar o componente personalizado ao pipeline do spaCy
    if "set_custom_boundaries" not in nlp_model.pipe_names:
        if "parser" in nlp_model.pipe_names:
            nlp_model.add_pipe("set_custom_boundaries", before="parser")
        elif "sentencizer" in nlp_model.pipe_names:
            nlp_model.add_pipe("set_custom_boundaries", before="sentencizer")
        else:
            nlp_model.add_pipe("set_custom_boundaries")


def split_sentences(texto: str):
    nlp_model = get_nlp()

    if nlp_model is None:
        return [parte.strip() for parte in re.split(r'(?<=[.!?])\s+', texto) if parte.strip()]

    ajustar_sentencizer(nlp_model)
    doc = nlp_model(texto)
...
def quebrar_mensagens(texto:str, probabilidade_quebra=0.5) -> list:
    ...
    try:
        ...
        if contains_markdown_list:
            ...
            for sent in split_sentences(texto):
                mensagem_atual += sent + " "
